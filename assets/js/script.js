...
var USR_VIN = "1C4PJLCB5HW650865"; //user input for vin number.  --- 
...
$.ajax({
    ...
    MODEL_YEAR = result.Results[0].ModelYear ;
    console.log("year: "+MODEL_YEAR);
    MAKE = result.Results[0].Make;
    console.log("make: "+MAKE);
    DRIVE_TYPE = result.Results[0].DriveType;
    console.log("drive type: "+DRIVE_TYPE);
    FUEL_TYPE = result.Results[0].FuelTypePrimary;
    console.log("fuel type: "+FUEL_TYPE);
    ERROR_CODE = result.Results[0].ErrorCode;
    console.log("error codes: "+ERROR_CODE);
    ERROR_TEXT = result.Results[0].ErrorText; 
    console.log("error text: "+ERROR_TEXT);
    ENGINE_CYLINDERS = result.Results[0].EngineCylinders;
    console.log("engine cylinders: "+ENGINE_CYLINDERS);
    ENGINE_HP = result.Results[0].EngineHP;
    console.log("horsepower: "+ENGINE_HP);
    SEATS = result.Results[0].Seats;  
    console.log("seats: "+SEATS);
    SEAT_ROWS = result.Results[0].SeatRows;
    console.log("seat rows: "+SEAT_ROWS);
    TOP_SPEED = result.Results[0].TopSpeedMPH; 
    console.log("top speed: "+TOP_SPEED);
...
// fetch("https://imsea.herokuapp.com/api/1?q="+ MAKE + MODEL)
// .then(function(){
